package org.azurite.ai.path;


import org.azurite.ai.path.vector2f.Vector2fMap;
import org.azurite.ai.path.vector2f.Vector2fNode;
...
public class PathfindingTest {
  ...
  public void preconditions() {
    ...
    Assertions.assertNotNull(map1.start());
    Assertions.assertNotNull(map1.target());
    Assertions.assertNotEquals(map1.start(), map1.target());
    //a test map should not end where it starts, this will confuse the algorithms in most cases
    Assertions.assertFalse(map1.start().paths().stream().anyMatch(path -> path.end().equals(map1.target())));
    Assertions.assertFalse(expectedPath1.isEmpty());
  }
  ...
  public void dijkstra1() {
    ...
    Assertions.assertTrue(dijkstra.isPresent());
    ResultPath<Vector2f> path = dijkstra.get();
    Assertions.assertNotNull(path);
    Assertions.assertNotNull(path.fullPath());
    Assertions.assertFalse(path.fullPath().isEmpty());
    String pathString = pathToString(path.fullPath());
    Assertions.assertEquals(expectedPath1, pathString);
  }
  ...
  public void astar1() {
    ...
    Assertions.assertTrue(astar.isPresent());
