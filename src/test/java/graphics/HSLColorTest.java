...
public class HSLColorTest {
  ...
  public void getHSLColor() {
    ...
    Assertions.assertEquals(color1, c1);
    Assertions.assertEquals(color2, c2);
    Assertions.assertEquals(color3, c3);
  }
  ...
  public void setAndGet() {
    ...
    Assertions.assertEquals(80, hslColor.get(HSLColor.LUMINANCE), delta(80));
  }

  @Test
  public void toHSLVector() {
    Assertions.assertEquals(new Vector3f(hslcolor1), color1.toHSLVector());
    Assertions.assertEquals(new Vector3f(hslcolor2), color2.toHSLVector());
    Assertions.assertEquals(new Vector3f(hslcolor3), color3.toHSLVector());
  }

  @Test
  public void toHSLAVector() {
    Assertions.assertEquals(new Vector4f(hslcolor1), color1.toHSLAVector());
    Assertions.assertEquals(new Vector4f(hslcolor2), color2.toHSLAVector());
    Assertions.assertEquals(new Vector4f(hslcolor3), color3.toHSLAVector());
  }
  ...
  public void toHSLA() {
    ...
    for (int i = 0; i < 4; i++) {
      Assertions.assertEquals(color1.get(i), hsla1[i], delta(color1.get(i)));
      Assertions.assertEquals(color2.get(i), hsla2[i], delta(color2.get(i)));
      Assertions.assertEquals(color3.get(i), hsla3[i], delta(color3.get(i)));
    }
  }
  ...
  public void toRGBA() {
    ...
    for (int i = 0; i < 4; i++) {
      Assertions.assertEquals(rgacolor1[i], rgba1[i] * 255, delta(rgacolor1[i]));
      Assertions.assertEquals(rgacolor2[i], rgba2[i] * 255, delta(rgacolor2[i]));
      Assertions.assertEquals(rgacolor3[i], rgba3[i] * 255, delta(rgacolor3[i]));
    }
  }
}