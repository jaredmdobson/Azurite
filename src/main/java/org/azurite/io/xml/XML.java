package org.azurite.io.xml;

import org.azurite.util.Pair;
...
public class XML {
    ...
    protected static List<XMLToken> tokenize(String input) {
        ...
        for (int i = 0; i < input.length(); i++) {
            switch (chars[i]) {
                case '<':
                    types[i] = XMLTokenType.OPEN_TAG;
                    break;
                case '>':
                    types[i] = XMLTokenType.CLOSE_TAG;
                    break;
                case '!':
                    types[i] = XMLTokenType.OPEN_COMMENT;
                    break;
                case '-':
                    types[i] = XMLTokenType.COMMENT_TAG;
                    break;
                case '=':
                    types[i] = XMLTokenType.ATTR_EQUALS;
                    break;
                case '/':
                    types[i] = XMLTokenType.SLASH;
                    break;
                case '"':
                    types[i] = XMLTokenType.QUOTATION;
                    break;
                default:
                    if (Character.isSpaceChar(chars[i]) || Character.isWhitespace(chars[i])) types[i] = XMLTokenType.SPACING;
                    else types[i] = XMLTokenType.IDENTIFIER;
                    break;
            }
        }
        ...
        for (int i = 0; i < types.length; i++) {
            switch (types[i]) {
                ...
                case COMMENT_TAG:
                    if (builder != null) {
                        if (identifier)
                            tokens.add(new XMLToken(builder.toString(), XMLTokenType.IDENTIFIER));
                        else tokens.add(new XMLToken(builder.toString(), XMLTokenType.SPACING));
                        builder = null;
                        identifier = false;
                    }
                ...
                case IDENTIFIER:
                    if (builder != null && !identifier) {
                        tokens.add(new XMLToken(builder.toString(), XMLTokenType.SPACING));
                        builder = null;
                    }
                    if (builder == null) {
