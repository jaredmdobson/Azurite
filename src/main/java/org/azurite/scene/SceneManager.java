...
public class SceneManager {
  ...
  public boolean addScene(Scene scene) {
    ...
    if (!add) Log.logger.warn("scene with id " + scene.sceneId() + " could not be added", 1);
    return add;
  }
  ...
  private void awaken(Scene scene) {
    if (enabled) {
      ...
      scene.startUi();
      scene.awake();
    } else Log.logger.warn("scene awaken called without being enabled", 1);
  }

  private boolean switchScene(Scene newCurrent, boolean newScene) {
    if (newCurrent == null) {
      Log.logger.warn("tried to switch to a scene that is null", 1);
      return false;
    }
    ...
    if (newCurrent == currentScene) {
      Log.logger.warn("tried to switch to current scene id " + currentScene.sceneId(), 1);
      return true;
    }
    ...
    Log.logger.info("switched to new scene (id: " + newCurrent.sceneId() + ")", 1);
    return true;
  }
